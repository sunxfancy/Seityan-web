    ...
    public async response(data: ZMReturn, query: string): Promise<string> {
        ...
        if (mode) {
            if (data.resultCode == "0000" && data.intents.length != 0 && data.intents[0].intent != "google") {
                ...
                window.open("https://www.google.com.hk/s?wd="+encodeURIComponent(query));
                return "为您找到这些信息";
            }
        }
        ...
        // 正确匹配
        window.open("https://www.google.com.hk/s?wd="+encodeURIComponent(data.entities[0].entity));
...
class BaiduSearch extends ZMPlugin {
    ...
    public async response(data: ZMReturn, query: string):  Promise<string> {
        ...
        // 正确匹配
        window.open("https://www.baidu.com/s?wd="+encodeURIComponent(data.entities[0].entity));
